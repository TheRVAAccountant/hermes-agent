...
class HermesCLI:
    ...
    def _handle_codex_compact(self) -> None:
        """Request compaction of the live Codex app-server thread."""
        agent = getattr(self, "agent", None)
        if agent is None:
            _cprint("  No active agent -- send a message first.")
            return
        handler = getattr(agent, "compact_codex_app_server", None)
        if handler is None:
            _cprint("  Active agent does not support Codex app-server compaction.")
            return
        ok, message = handler()
        prefix = "✓" if ok else "✗"
        _cprint(f"  {prefix} {message}")
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif canonical == "codex-compact":
            self._handle_codex_compact()
