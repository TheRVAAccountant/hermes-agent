...
def test_cli_init_starts_named_local_model_server(monkeypatch):
    cli = _import_cli()
    ...
    monkeypatch.setattr(cli, "CLI_CONFIG", config_copy)
    monkeypatch.setattr("hermes_cli.local_model_servers.start_server_action", lambda action: calls.append(action))

    shell = cli.HermesCLI(
        model=f"custom:local-mlx-gemma4-4bit:{model_path}",
        compact=True,
        max_turns=1,
    )

    assert shell.requested_provider == "custom:local-mlx-gemma4-4bit"
    assert shell.model == model_path
    assert calls == ["gemmamlx"]


def test_cli_cleanup_stops_active_local_model_server(monkeypatch):
    cli = _import_cli()
    calls = []
    monkeypatch.setattr("hermes_cli.local_model_servers.stop_active_local_server", lambda: calls.append("stop") or True)

    shell = cli.HermesCLI(model="gpt-5", compact=True, max_turns=1)
    shell._cleanup_local_model_server()

    assert calls == ["stop"]


def test_cli_init_parses_named_custom_provider_model(monkeypatch):
    cli = _import_cli()
    ...
    shell = cli.HermesCLI(
        model="custom:local-llamacpp-gemma4-gguf:gemma-4-26B-A4B-it-GGUF",
        compact=True,
        max_turns=1,
    )

    assert shell.requested_provider == "custom:local-llamacpp-gemma4-gguf"
    assert shell.provider == "custom:local-llamacpp-gemma4-gguf"
    assert shell.model == "gemma-4-26B-A4B-it-GGUF"
