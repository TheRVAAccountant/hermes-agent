...
class TestCodexAppServerModule:
    ...
    def test_pending_request_fails_when_subprocess_exits_during_initialize(
        self, monkeypatch
    ) -> None:
        """If codex exits before replying to initialize, the client should
        surface the child-exit/stderr cause instead of waiting for a generic
        request timeout. This is the failure shape behind Hermes reporting
        "initialize timed out" while stderr/stdin show the child already died.
        """
        import subprocess
        ...
        class ExitedPopen:
            def __init__(self, *args, **kwargs):
                ...
                self.stderr = FakePipe(
                    [b"failed to load configuration: duplicate key\n"]
                )
                self.returncode = 1
        ...
        with pytest.raises(RuntimeError) as excinfo:
            client.request("initialize", {"clientInfo": {}}, timeout=0.05)

        message = str(excinfo.value)
        assert "initialize" in message
        assert "subprocess" in message or "exited" in message
