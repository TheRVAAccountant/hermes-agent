...
ZAYA_MODEL_PATH = "/Users/jeroncrooks/ZAYA1-8B-JANGTQ4"
...
def test_switch_model_starts_zaya_server_action_before_validation(monkeypatch):
    calls = []
    custom_providers = [
        {
            "name": "local-mlx-zaya1-jangtq4",
            "base_url": "http://localhost:8095/v1",
            "model": ZAYA_MODEL_PATH,
            "models": {ZAYA_MODEL_PATH: {"context_length": 131072}},
            "server_action": "zaya1jangtq",
        }
    ]

    def fake_runtime_provider(**kwargs):
        return {
            "provider": "custom:local-mlx-zaya1-jangtq4",
            "api_key": "no-key-required",
            "base_url": "http://localhost:8095/v1",
            "api_mode": "chat_completions",
        }
    ...
    result = switch_model(
        ...
        explicit_provider="custom:local-mlx-zaya1-jangtq4",
        custom_providers=custom_providers,
    )

    assert result.success is True
    assert calls[0] == ("start", "zaya1jangtq")
    assert calls[1][0] == "validate"
