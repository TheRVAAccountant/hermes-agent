    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        ...
        for entry in get_compatible_custom_providers(cfg):
            ...
            custom_provider_map[key] = {
                ...
                "server_action": entry.get("server_action", ""),
...
def _model_flow_named_custom(config, provider_info):
    ...
    server_action = (provider_info.get("server_action") or "").strip()
    ...
    if server_action:
        print(f"  Startup:  {server_action}")
    print()

    if server_action:
        from hermes_cli.local_model_servers import start_server_action

        print("Starting local model server...")
        try:
            start_server_action(server_action)
        except Exception as exc:
            print(f"Warning: could not start local model server '{server_action}': {exc}")
