    ...
    if name == "codex-compact":
        ...
        if session.get("running"):
            return _err(
                rid,
                4009,
                "session busy -- /interrupt the current turn before /codex-compact",
            )
        agent = session.get("agent")
        handler = getattr(agent, "compact_codex_app_server", None) if agent else None
        if handler is None:
            return _err(rid, 4018, "no active Codex app-server agent")
        ok, message = handler()
        if ok:
            _emit("session.info", params.get("session_id", ""), _session_info(agent))
